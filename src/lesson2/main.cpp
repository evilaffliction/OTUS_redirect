...
// здесь мы храним все сохраненные адреса
int counter;
...
class Save_resource: public httpserver::http_resource {
    ...
    const http_response render_POST(const http_request& req) {
        ...
        mtx.lock();
        // запоминаем новое количество url-ов
        int currentNumber = counter++;
        // добавляем нам новый URL
...
class Redirect_resource: public httpserver::http_resource {
    ...
    const http_response render_GET(const httpserver::http_request& req) {
        ...
        std::string val = req.get_path_piece(1);
        std::cout << YELLOW << "Параметр в пути запроса = " << BLUE << val << std::endl;
        int id = 0;
        try {
            id  = std::stoi(val);
        }
            ...
            httpserver::string_response* response = 
                new httpserver::string_response("Invalid key " + val, 400);
            std::cout << RED << "Вызов функции Redirect_resource::render завершен c ошибкой" << std::endl;
            return http_response(response);
        }
        ...
        std::string responseUrl = redirectUrls[id];
        mtx.unlock();

        httpserver::string_response* response = new httpserver::string_response("", 302);
        response->with_header("Location", responseUrl);
